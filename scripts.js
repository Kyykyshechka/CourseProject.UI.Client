...
const connection = new signalR.HubConnectionBuilder()
    .withUrl("https://localhost:7096/indicator")
...
connection.on("receive", (id, value) =>
  {
    ...
    if (id === "0e990ade-b468-4ec3-d9ab-08dd18372fe9") {
        updateTemperature(parseFloat(value));
    }
    if (id === "1be1c745-43e5-46ac-7c8e-08dd18439072") {
        updatePressure(parseFloat(value));
    }
    if (id === "0e990ade-b468-4ec3-d9ab-08dd18372fe9") {
        updateATemperature(parseFloat(value));
    }
    if (id === "1be1c745-43e5-46ac-7c8e-08dd18439072") {
        updateAPressure(parseFloat(value));
    }
...
function getIndicators(){
    axios.get('https://localhost:7096/api/Indicator')
    .then(result =>{
        console.log(result);
...
function tryNewIndicator(name, value, unit, x, y)
{
    ...
    axios.post('https://localhost:7096/api/Indicator', {
...
function changeBackgroundImage(id)
{
    ...
    element.src = 'https://localhost:7096/api/BackgroundImage/' + id;
    console.log(element)
}
...
document.getElementById("imageForm").addEventListener("submit", function(event){
    ...
    if(fileInput)
    {
        ...
        axios.post('https://localhost:7096/api/backgroundimage/upload-image', formData)
        .then(result =>{
...
function onDelete(e, id){
    axios.delete(`https://localhost:7096/api/indicator/${id}`);
...
function updateTemperature(temperature) {
    const temperatureValue = document.getElementById('burner-value');
    const temperatureLight = document.getElementById('burner-light');

    temperatureValue.textContent = temperature;

    if (temperature < 800) {
        temperatureLight.style.backgroundColor = 'red'; 
    } else {
        temperatureLight.style.backgroundColor = 'green';
    }
}

function updatePressure(pressure) {
    const pressureValue = document.getElementById('fan-value');
    const pressureLight = document.getElementById('fan-light');

    pressureValue.textContent = pressure;

    if (pressure > 180) {
        pressureLight.style.backgroundColor = 'red'; 
    } else {
        pressureLight.style.backgroundColor = 'green';
    }
}

function updateATemperature(atemperature) {
    const atemperatureValue = document.getElementById('alarmtemperature-value');
    const atemperatureLight = document.getElementById('alarmtemperature-light');

    atemperatureValue.textContent = atemperature;

    if (atemperature > 1200) {
        atemperatureLight.style.backgroundColor = 'red'; 
    } else {
        atemperatureLight.style.backgroundColor = 'green';
    }
}

function updateAPressure(apressure) {
    const apressureValue = document.getElementById('alarmpressure-value');
    const apressureLight = document.getElementById('alarmpressure-light');

    apressureValue.textContent = apressure;

    if (apressure < 160) {
        apressureLight.style.backgroundColor = 'red'; 
    } else {
        apressureLight.style.backgroundColor = 'green';
    }
}